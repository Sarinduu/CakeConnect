...
const BakerDirectoryPage = () => {
  ...
  const filteredBakers =
    filter === "all"
      ? bakers
      : bakers.filter((b) => b.bakerType === filter);

  return (
    <div className="baker-directory">
      <h2>Explore Our Bakers</h2>
      {message && <div className="baker-directory-message">{message}</div>}

      <div className="baker-directory-controls">
      ...
      {selectedBaker && (
        <div className="baker-popup">
          <div className="popup-inner">
            <button className="close-btn" onClick={() => setSelectedBaker(null)}>
              ×
            </button>
            <div className="popup-left">
              <img src={selectedBaker.imageUrl} alt={selectedBaker.name} />
              <h3>{selectedBaker.name}</h3>
              <p><strong>Type:</strong> {selectedBaker.bakerType}</p>
              <p><strong>Specialty:</strong> {selectedBaker.specialty}</p>
              <p><strong>Location:</strong> {selectedBaker.location}</p>
              <p><strong>Experience:</strong> {selectedBaker.experience}</p>
              <p><strong>Rating:</strong> {selectedBaker.rating} ★</p>
            </div>
